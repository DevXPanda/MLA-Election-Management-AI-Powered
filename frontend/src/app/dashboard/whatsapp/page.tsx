...
export default function WhatsAppPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      {/* ─── MODULE HEADER ─── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-dark-100 dark:border-white/[0.04] pb-6">
        <div>
          ...
          <p className="text-sm text-dark-500 dark:text-dark-400 mt-1">{header.subtitle}</p>
        </div>
        
        {/* Navigation Tabs */}
        <div className="flex items-center bg-dark-50 dark:bg-dark-900/50 p-1 rounded-xl border border-dark-200/50 dark:border-white/[0.04]">
          <button
            onClick={() => setActiveTab('analytics')}
            className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all ${activeTab === 'analytics' ? 'bg-white dark:bg-dark-800 text-saffron-600 dark:text-saffron-400 shadow-sm' : 'text-dark-600 hover:text-dark-900 dark:text-dark-400 dark:hover:text-dark-200'}`}
          >
            {t('whatsapp.analytics', 'Analytics')}
          </button>
          <button
            onClick={() => setActiveTab('compose')}
            className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all ${activeTab === 'compose' ? 'bg-white dark:bg-dark-800 text-saffron-600 dark:text-saffron-400 shadow-sm' : 'text-dark-600 hover:text-dark-900 dark:text-dark-400 dark:hover:text-dark-200'}`}
          >
            {t('whatsapp.new_campaign', 'New Campaign')}
          </button>
          <button
            onClick={() => setActiveTab('history')}
            className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all ${activeTab === 'history' ? 'bg-white dark:bg-dark-800 text-saffron-600 dark:text-saffron-400 shadow-sm' : 'text-dark-600 hover:text-dark-900 dark:text-dark-400 dark:hover:text-dark-200'}`}
          >
            {t('whatsapp.history', 'History')}
          </button>
          <button
            onClick={() => setActiveTab('templates')}
            className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all ${activeTab === 'templates' ? 'bg-white dark:bg-dark-800 text-saffron-600 dark:text-saffron-400 shadow-sm' : 'text-dark-600 hover:text-dark-900 dark:text-dark-400 dark:hover:text-dark-200'}`}
          >
          ...
          {user?.role_name === 'super_admin' && (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all ${activeTab === 'settings' ? 'bg-white dark:bg-dark-800 text-saffron-600 dark:text-saffron-400 shadow-sm' : 'text-dark-600 hover:text-dark-900 dark:text-dark-400 dark:hover:text-dark-200'}`}
            >
              {t('whatsapp.settings', 'API Settings')}
            </button>
          )}
        </div>
      </div>
      ...
      {/* ─── TAB 5: SETTINGS ─── */}
      {activeTab === 'settings' && user?.role_name === 'super_admin' && (
        <div className="bg-white dark:bg-dark-900/50 rounded-2xl border border-dark-200/50 dark:border-white/[0.04] p-6 animate-fade-in space-y-6">
          <div className="border-b border-dark-100 dark:border-white/[0.04] pb-4">
            <h3 className="font-bold text-dark-900 dark:text-white flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-saffron-500" />
              WhatsApp Business API Settings
            </h3>
            <p className="text-xs text-dark-500 dark:text-dark-400 mt-0.5">Configure your Meta Graph API credentials and webhook integration details.</p>
          </div>
          ...
          <form onSubmit={handleSaveSettings} className="space-y-6 max-w-2xl">
            ...
            {/* Access Token */}
            <div className="space-y-1">
              <label className="block text-xs font-semibold text-dark-700 dark:text-dark-400">Meta Access Token (Permanent Page Access Token) *</label>
              <div className="relative">
                <input
                  ...
                  placeholder="EAAG..."
                  required
                  className="w-full pl-3 pr-10 py-2 bg-dark-50 dark:bg-dark-900 rounded-lg border border-dark-200/60 dark:border-white/[0.04] text-xs text-dark-900 dark:text-white focus:outline-none focus:border-saffron-500 font-mono"
                />
                <button
                  type="button"
                  onClick={() => setShowToken(!showToken)}
                  className="absolute inset-y-0 right-0 pr-3 flex items-center text-dark-400 hover:text-dark-600 dark:hover:text-dark-200 text-xs font-semibold"
                >
                  {showToken ? "Hide" : "Show"}
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Phone Number ID */}
              <div className="space-y-1">
                <label className="block text-xs font-semibold text-dark-700 dark:text-dark-400">Phone Number ID *</label>
                <input
                  type="text"
                  value={phoneNumberId}
                  onChange={(e) => setPhoneNumberId(e.target.value)}
                  placeholder="e.g., 105658742512345"
                  required
                  className="w-full px-3 py-2 bg-dark-50 dark:bg-dark-900 rounded-lg border border-dark-200/60 dark:border-white/[0.04] text-xs text-dark-900 dark:text-white focus:outline-none focus:border-saffron-500 font-mono"
                />
              </div>

              {/* Business Account ID */}
              <div className="space-y-1">
                <label className="block text-xs font-semibold text-dark-700 dark:text-dark-400">WhatsApp Business Account ID *</label>
                <input
                  type="text"
                  value={businessAccountId}
                  onChange={(e) => setBusinessAccountId(e.target.value)}
                  placeholder="e.g., 109854742512345"
                  required
                  className="w-full px-3 py-2 bg-dark-50 dark:bg-dark-900 rounded-lg border border-dark-200/60 dark:border-white/[0.04] text-xs text-dark-900 dark:text-white focus:outline-none focus:border-saffron-500 font-mono"
                />
              </div>
            ...
            <div className="space-y-1">
              <label className="block text-xs font-semibold text-dark-700 dark:text-dark-400">Webhook Verification Token *</label>
              <div className="relative">
                <input
                  type={showVerifyToken ? "text" : "password"}
                  value={webhookVerifyToken}
                  onChange={(e) => setWebhookVerifyToken(e.target.value)}
                  placeholder="Create a secure token for Meta Webhook verification"
                  required
                  className="w-full pl-3 pr-10 py-2 bg-dark-50 dark:bg-dark-900 rounded-lg border border-dark-200/60 dark:border-white/[0.04] text-xs text-dark-900 dark:text-white focus:outline-none focus:border-saffron-500 font-mono"
                />
                <button
                  ...
                  className="absolute inset-y-0 right-0 pr-3 flex items-center text-dark-400 hover:text-dark-600 dark:hover:text-dark-200 text-xs font-semibold"
                >
                  {showVerifyToken ? "Hide" : "Show"}
                </button>
              </div>
              <p className="text-[10px] text-dark-500 dark:text-dark-400">This token must match the verify token you set up in the Meta Developer Console Webhook settings.</p>
            </div>

            {/* Callback URL Display */}
            <div className="p-4 bg-dark-50 dark:bg-dark-950/20 border border-dark-100 dark:border-white/[0.02] rounded-xl space-y-2">
              <span className="block text-xs font-bold text-dark-700 dark:text-dark-400 uppercase tracking-wider">Meta Webhook Configuration Details</span>
              <div className="space-y-1.5 text-xs text-dark-600 dark:text-dark-400">
                <p>To receive real-time updates (Delivered, Read, Failed statuses), configure the following values in your Meta Application Developer Portal:</p>
                <div className="grid grid-cols-3 gap-2 pt-2 items-center">
                  <span className="font-semibold text-dark-700 dark:text-dark-300">Callback URL:</span>
                  <code className="col-span-2 p-1.5 bg-dark-100 dark:bg-dark-900 rounded text-saffron-600 dark:text-saffron-400 font-mono select-all text-[11px] truncate">
                    {webhookUrl || '/api/whatsapp/webhook'}
                  </code>
                </div>
                <div className="grid grid-cols-3 gap-2 items-center">
                  <span className="font-semibold text-dark-700 dark:text-dark-300">Verify Token:</span>
                  <span className="col-span-2 p-1.5 bg-dark-100 dark:bg-dark-900 rounded text-dark-800 dark:text-dark-200 font-mono text-[11px] select-all">
                    {webhookVerifyToken || 'Configure token above'}
                  </span>
                </div>
              </div>
            </div>
            ...
            {/* Save Button */}
            <div className="pt-4 border-t border-dark-100 dark:border-white/[0.04] flex justify-end">
              <button
                type="submit"
                disabled={savingSettings}
                className="px-6 h-10 bg-gradient-to-r from-saffron-500 to-amber-600 hover:from-saffron-600 hover:to-amber-700 text-white font-bold rounded-xl shadow-lg shadow-saffron-500/10 flex items-center justify-center gap-2 transition-all disabled:opacity-50 text-xs"
              >
                {savingSettings ? (
                  <>
                    <RefreshCw className="w-4 h-4 animate-spin" />
