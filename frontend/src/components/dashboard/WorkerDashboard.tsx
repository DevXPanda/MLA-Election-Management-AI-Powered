...
export default function WorkerDashboard({ stats }: DashboardProps) {
   ...
   const statCards = [
      { label: t('worker.active_tasks', 'Active Tasks'), value: my_pending_tasks || 0, icon: ListTodo, color: 'text-amber-400', bgIcon: 'bg-amber-500/12' },
      { label: t('worker.tasks_done', 'Tasks Done'), value: my_completed_tasks || 0, icon: CheckCircle2, color: 'text-emerald-400', bgIcon: 'bg-emerald-500/12' },
      { label: t('worker.total_surveys', 'Total Surveys'), value: total_surveys_submitted || 0, icon: ClipboardPlus, color: 'text-blue-400', bgIcon: 'bg-blue-500/12' },
      { label: t('worker.today', 'Today'), value: surveys_today || 0, icon: Zap, color: 'text-purple-400', bgIcon: 'bg-purple-500/12' },
   ];
         ...
         <div className="gradient-card p-7 mb-8 relative group overflow-hidden border border-saffron-500/20 rounded-lg shadow-2xl shadow-saffron-500/10 hover:shadow-saffron-500/20 transition-all">
            <div className="relative z-10">
               <h2 className="text-2xl font-black text-dark-900 dark:text-white flex items-center gap-4 mb-0">
                  <Zap className="w-7 h-7 text-saffron-500 animate-pulse" /> {t('worker.my_field_ops', 'My Field Operations')}
               </h2>
               <p className="text-sm text-dark-600 dark:text-dark-400 max-w-sm mt-2 font-medium leading-relaxed">
                  {t('worker.field_ops_desc', 'Help your booth reach 100% coverage by performing regular surveys and tasks.')}
               </p>
               <div className="flex flex-wrap gap-4 mt-8">
                  <Link href="/dashboard/surveys" className="inline-flex items-center gap-3 px-8 py-4 bg-saffron-400 text-dark-900 font-black text-xs uppercase tracking-[2px] rounded-lg shadow-xl shadow-saffron-500/30 hover:scale-[1.05] hover:shadow-saffron-500/50 transition-all transform active:scale-95 group/btn">
                     {t('worker.submit_survey', 'Submit Survey')} <ClipboardPlus className="w-5 h-5 group-hover/btn:rotate-12 transition-transform" />
                  </Link>
                  <Link href="/dashboard/voters" className="inline-flex items-center gap-3 px-8 py-4 bg-white/5 text-dark-100 font-black text-xs uppercase tracking-[2px] rounded-lg border border-white/5 hover:bg-white/10 transition-all active:scale-95">
                     {t('worker.search_voters', 'Search Voters')} <ArrowRight className="w-4 h-4" />
                  </Link>
               </div>
            </div>
            ...
            <div className="glass-card overflow-hidden flex flex-col">
               <div className="p-6 border-b border-white/5 flex items-center justify-between">
                  <h3 className="text-base font-black text-dark-900 dark:text-white flex items-center gap-3">
                     <ListTodo className="w-5 h-5 text-amber-400" /> {t('worker.my_priority_assignment', 'My Priority Assignment')}
                  </h3>
                  <Link href="/dashboard/tasks" className="text-[10px] font-black text-saffron-400 uppercase tracking-widest hover:text-saffron-300 transition-colors bg-saffron-400/5 px-3 py-1 rounded-full border border-saffron-500/10">{t('worker.manage_all', 'Manage All')}</Link>
               </div>
               <div className="p-4 space-y-4">
                     ...
                     <div key={task.id} className="p-5 rounded-lg bg-dark-800/20 hover:bg-dark-800/40 transition-all border border-white/[0.03] group flex items-center justify-between">
                        <div className="min-w-0">
                           ...
                           <div className="text-[10px] text-dark-600 dark:text-dark-500 font-bold uppercase flex items-center gap-3 tracking-widest">
                              ...
                              <span className="opacity-60">{t('priority.' + task.priority, task.priority)}</span>
                           </div>
                        </div>
                  ...
                  {(!stats.lists.tasks || stats.lists.tasks.length === 0) && (
                     <div className="py-16 flex flex-col items-center justify-center opacity-40">
                        ...
                        <p className="text-sm font-black uppercase tracking-widest">{t('worker.targets_cleared', 'Targets Cleared')}</p>
                     </div>
                  )}
               </div>
            </div>
