...
export default function MlaDashboard({ stats, chartDefaults }: DashboardProps) {
  ...
  const statCards = [
    { label: t('dashboard.total_voters', 'Total Voters'), value: stats.stats.total_voters, icon: Vote, color: 'text-blue-400', bgIcon: 'bg-blue-500/12' },
    { label: t('dashboard.survey_coverage', 'Survey Coverage %'), value: Math.round((stats.stats.total_surveys / (stats.stats.total_voters || 1)) * 100) + '%', icon: Target, color: 'text-purple-400', bgIcon: 'bg-purple-500/12' },
    { label: t('dashboard.strong_booths', 'Strong Booths'), value: booth_strength?.strong_booths || 0, icon: TrendingUp, color: 'text-green-400', bgIcon: 'bg-green-500/12' },
    { label: t('dashboard.weak_booths', 'Weak Booths'), value: booth_strength?.weak_booths || 0, icon: ShieldAlert, color: 'text-red-400', bgIcon: 'bg-red-500/12' },
    { label: t('dashboard.active_staff', 'Active Staff'), value: stats.stats.active_workers, icon: Activity, color: 'text-emerald-400', bgIcon: 'bg-emerald-500/12' },
  ];

  const boothStrengthData = {
    labels: [
      t('dashboard.booth.strong', 'Strong'), 
      t('dashboard.booth.competitive', 'Competitive'), 
      t('dashboard.booth.weak', 'Weak')
    ],
    datasets: [{
      data: [
  ...
  const issuesData = {
    labels: top_issues?.map(i => t(i.name.toLowerCase(), i.name)) || [],
    datasets: [{
      label: t('dashboard.key_issues', 'Key Constituency Issues'),
  ...
  return (
    <div className="dashboard-container pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="glass-card p-6">
          <h3 className="text-base font-medium text-dark-900 dark:text-white mb-6 flex items-center gap-2">
            <MapIcon className="w-5 h-5 text-saffron-500" /> {t('dashboard.booth_strength_title', 'Booth Strength')}
          </h3>
          ...
          <div className="mt-8 space-y-4">
            <div className="flex justify-between items-center bg-green-500/5 p-3 rounded-lg border border-green-500/10">
              <span className="text-[10px] font-medium text-green-600 dark:text-green-400 uppercase tracking-widest">{t('dashboard.booth.strong_label', 'Strong (>60%)')}</span>
            ...
            <div className="flex justify-between items-center bg-amber-500/5 p-3 rounded-lg border border-amber-500/10">
              <span className="text-[10px] font-medium text-amber-600 dark:text-amber-400 uppercase tracking-widest">{t('dashboard.booth.competitive_label', 'Competitive (40-60)')}</span>
            ...
            <div className="flex justify-between items-center bg-red-500/5 p-3 rounded-lg border border-red-500/10">
              <span className="text-[10px] font-medium text-red-600 dark:text-red-400 uppercase tracking-widest">{t('dashboard.booth.weak_label', 'Weak (<40%)')}</span>
        ...
        <div className="glass-card p-6 border border-dark-100/50 dark:border-white/5 shadow-xl bg-gradient-to-br from-indigo-500/[0.02] to-transparent">
          <h3 className="text-base font-medium text-dark-900 dark:text-white mb-8 flex items-center gap-2">
             <BarChart3 className="w-5 h-5 text-indigo-500" /> {t('dashboard.key_issues', 'Key Constituency Issues')}
          </h3>
              ...
              <div key={i} className="relative">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[11px] font-bold text-dark-700 dark:text-dark-300 uppercase tracking-widest flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" /> {t(issue.name.toLowerCase(), issue.name)}
                  </span>
      ...
      {stats.lists.work_allocations && stats.lists.work_allocations.length > 0 && (
        <div className="glass-card p-6 border border-dark-100/50 dark:border-white/5 shadow-xl mb-6 bg-gradient-to-br from-saffron-500/[0.01] to-transparent">
          <h3 className="text-base font-medium text-dark-900 dark:text-white mb-6 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card overflow-hidden">
          <div className="p-6 border-b border-dark-100 dark:border-white/5 flex items-center justify-between">
            <h3 className="text-base font-medium text-dark-900 dark:text-white flex items-center gap-2">
              <Users className="w-5 h-5 text-emerald-500" /> {t('dashboard.field_performance', 'Field Performance')}
            </h3>
            <span className="text-[10px] font-bold text-dark-500 uppercase tracking-widest">{t('dashboard.top_contributors', 'Top Contributors')}</span>
          </div>
          <div className="p-4 space-y-2">
            {stats.lists.top_performers?.map((worker, i) => (
              <div key={worker.name} className="flex items-center justify-between p-3 rounded-xl hover:bg-dark-50 dark:hover:bg-white/5 transition-colors">
                ...
                <div className="text-[11px] font-medium text-green-600 dark:text-green-400 capitalize bg-green-500/10 px-3 py-1 rounded-full">{worker.surveys_count} {t('sidebar.surveys', 'Surveys')}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-card p-6 flex flex-col bg-gradient-to-br from-saffron-500/[0.03] to-transparent border border-saffron-500/10">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h4 className="text-sm font-bold text-dark-900 dark:text-white uppercase tracking-wider leading-none">{t('dashboard.constituency_target', 'Constituency Target')}</h4>
              <p className="text-[10px] text-dark-500 mt-1 uppercase tracking-widest">{t('dashboard.election_goal_2026', 'Election Goal 2026')}</p>
            </div>
          </div>
          ...
          <div className="space-y-6 flex-grow flex flex-col justify-center">
             <div>
                <div className="flex justify-between items-end mb-3">
                   <span className="text-xs font-bold text-dark-600 dark:text-dark-400 uppercase tracking-[2px]">{t('dashboard.booth_penetration', 'Booth Penetration')}</span>
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-white/50 dark:bg-white/5 border border-dark-100 dark:border-white/5">
                   <div className="text-[10px] font-bold text-dark-500 uppercase tracking-widest mb-1">{t('dashboard.target_voters', 'Target Voters')}</div>
                   <div className="text-xl font-black text-dark-900 dark:text-white">14.2K</div>
                </div>
                <div className="p-4 rounded-2xl bg-white/50 dark:bg-white/5 border border-dark-100 dark:border-white/5">
                   <div className="text-[10px] font-bold text-dark-500 uppercase tracking-widest mb-1">{t('dashboard.reach_gap', 'Reach Gap')}</div>
